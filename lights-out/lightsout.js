...
Elm.LightsOut = Elm.LightsOut || {};
Elm.LightsOut.make = function (_elm) {
   "use strict";
   _elm.LightsOut = _elm.LightsOut || {};
   if (_elm.LightsOut.values) return _elm.LightsOut.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var foldp$ = F3(function (fun,initFun,input) {
      ...
      var initial = A2($Signal.map,initFun,A2($Signal.sampleOn,$Signal.constant({ctor: "_Tuple0"}),input));
      var rest = A3($Signal.foldp,fun$,$Maybe.Nothing,A3($Signal.map2,F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),input,initial));
      return A2($Signal.map,unsafe,A2($Signal.merge,A2($Signal.map,$Maybe.Just,initial),rest));
   });
   var toToggle = F2(function (m,_p4) {
      var _p5 = _p4;
      var _p9 = _p5._1;
      var _p8 = _p5._0;
      var sumTup = function (_p6) {    var _p7 = _p6;return {ctor: "_Tuple2",_0: _p8 + _p7._0,_1: _p9 + _p7._1};};
      return _U.cmp(A2($Basics.min,_p8,_p9),0) < 0 || _U.cmp(A2($Basics.max,_p8,_p9),m.level) > -1 ? $Maybe.Nothing : _U.eq(m.level,
      6) ? $Maybe.Just(A2($List.map,
      sumTup,
      _U.list([{ctor: "_Tuple2",_0: 0,_1: 0}
              ,{ctor: "_Tuple2",_0: -1,_1: -1}
              ,{ctor: "_Tuple2",_0: -1,_1: 1}
              ,{ctor: "_Tuple2",_0: 1,_1: 1}
              ,{ctor: "_Tuple2",_0: 1,_1: -1}]))) : $Maybe.Just(A2($List.map,
      sumTup,
      _U.list([{ctor: "_Tuple2",_0: 0,_1: 0}
   ...
   var togglePoint = F2(function (togglePoints,p) {
      return A2($List.member,{ctor: "_Tuple2",_0: p.i,_1: p.j},togglePoints) ? _U.update(p,{clicked: $Basics.not(p.clicked)}) : p;
   });
   var randomClick = F2(function (t,m) {
      ...
      var rnd = _p10._0;
      var s = _p10._1;
      var toggle = A2(toToggle,m,{ctor: "_Tuple2",_0: rnd / m.level | 0,_1: A2($Basics._op["%"],rnd,m.level)});
      var _p11 = toggle;
      if (_p11.ctor === "Just") {
            return _U.update(m,
            {points: A2($List.map,togglePoint(_p11._0),m.points)
   ...
   var randomize = F2(function (input,model) {
      if ($Basics.not(model.randomizing)) return model; else {
            var _p12 = input;
            if (_p12.ctor === "Tick") {
                  return A2(randomClick,_p12._0,model);
               } else {
                  return model;
               }
         }
   });
   var txt = F2(function (c,string) {    return $Graphics$Element.centered($Text.monospace(A2($Text.color,c,$Text.fromString(string))));});
   var resetBtnMargin = function (h) {    return $Basics.round($Basics.toFloat(h) * 0.2);};
   var boardSize = function (model) {    return A2(F2(function (x,y) {    return x * y;}),0.6,$Basics.toFloat(A2($Basics.uncurry,$Basics.min,model.window)));};
   var mouseToIndex = F2(function (mouse,model) {
      var _p13 = function (_p14) {    var _p15 = _p14;return {ctor: "_Tuple2",_0: $Basics.toFloat(_p15._0),_1: $Basics.toFloat(_p15._1)};}(mouse);
      var x = _p13._0;
      var y = _p13._1;
      var bsize = boardSize(model);
      var squareSize = bsize / $Basics.toFloat(model.level);
      var _p16 = model.window;
      var w = _p16._0;
      var h = _p16._1;
      var rmargin = $Basics.toFloat(resetBtnMargin(h));
      var _p17 = {ctor: "_Tuple2",_0: x - ($Basics.toFloat(w) - bsize + squareSize) / 2,_1: 0 - y + ($Basics.toFloat(h) + bsize - squareSize - rmargin) / 2};
      var i = _p17._0;
      var j = _p17._1;
      return {ctor: "_Tuple2",_0: $Basics.round(i / squareSize),_1: $Basics.round(j / squareSize)};
   });
   var Model = F7(function (a,b,c,d,e,f,g) {    return {points: a,clicks: b,window: c,randomizing: d,rndSetup: e,ended: f,level: g};});
   var initRndSetup = function (s) {
      return {clicks: $Basics.round($Basics.toFloat(s) * $Basics.toFloat(s) * 0.4),seed: $Random.initialSeed(-1),rndInt: -1};
   };
   var RndSetup = F3(function (a,b,c) {    return {clicks: a,seed: b,rndInt: c};});
   var buildPoint = F2(function (i,j) {    return {i: i,j: j,hover: 0,highlight: 0,clicked: false,toggled: false};});
   var buildGrid = function (s) {
      return A2($List.map,
      function (_p18) {
         return A2($Basics.uncurry,buildPoint,function (a) {    return {ctor: "_Tuple2",_0: a / s | 0,_1: A2($Basics._op["%"],a,s)};}(_p18));
      },
      _U.range(0,s * s - 1));
   };
   var initialModel = F2(function (level,input) {
      var win$ = function () {    var _p19 = input;if (_p19.ctor === "Resize") {    return _p19._0;} else {    return {ctor: "_Tuple2",_0: 0,_1: 0};}}();
      return {points: buildGrid(level),clicks: 0,window: win$,randomizing: true,rndSetup: initRndSetup(level),ended: false,level: level};
   });
   var Point = F6(function (a,b,c,d,e,f) {    return {i: a,j: b,hover: c,highlight: d,clicked: e,toggled: f};});
   var Reset = {ctor: "Reset"};
   var resetButton = $Signal.mailbox(Reset);
   var Tick = function (a) {    return {ctor: "Tick",_0: a};};
   var Resize = function (a) {    return {ctor: "Resize",_0: a};};
   var play = F2(function (input,model) {
      if (model.randomizing) return model; else if (model.ended) {
               ...
               var _p21 = input;
               switch (_p21.ctor)
               {case "Move": return _U.update(model,
                    {points: A2($List.map,
                    function (p) {
                       return _U.update(p,{hover: _U.eq({ctor: "_Tuple2",_0: p.i,_1: p.j},A2(mouseToIndex,_p21._0,model)) ? 1 : 0});
                    },
                    model.points)});
                  case "Click": var _p22 = A2(toToggle,model,A2(mouseToIndex,_p21._0,model));
                    if (_p22.ctor === "Just") {
                          var points = A2($List.map,togglePoint(_p22._0),model.points);
                          return _U.update(model,
                          {points: points
                          ,ended: A2($List.all,function (_p23) {    return $Basics.not(function (_) {    return _.clicked;}(_p23));},points)
